<script lang="ts">
  ...
  import { Mail, RefreshCw, CheckCircle, AlertCircle, ArrowRight, Home, Inbox } from 'lucide-svelte';

  // Email passed via query param e.g. /verify-email?email=user@example.com
  ...
  onMount(() => {
    // If they landed here right after signup, start cooldown immediately
    startCooldown();
  });
  ...
  // Progress dots animation state
  let activeDot = $state(0);
  let dotTimer: ReturnType<typeof setInterval>;
  onMount(() => {
    dotTimer = setInterval(() => { activeDot = (activeDot + 1) % 3; }, 900);
  });
  onDestroy(() => clearInterval(dotTimer));

  // Cooldown ring — percentage remaining
  const ringProgress = $derived(cooldown > 0 ? (cooldown / COOLDOWN_SECS) * 100 : 0);
  const circumference = 2 * Math.PI * 20; // r=20
  const strokeDash = $derived(circumference - (ringProgress / 100) * circumference);
</script>
    ...
    <div class="ve-card">
      ...
      <div class="ve-card-content">
        ...
        <p class="ve-subtitle">
          ...
          <strong class="ve-email">{maskedEmail()}</strong>
        </p>
        ...
        <div class="ve-resend-wrap">
          ...
          <div class="ve-resend-row">
            ...
            <button
              ...
              {#if resending}
                <span class="ve-spin"><RefreshCw size={13} /></span>
                <span>Sending…</span>
              {:else if cooldown > 0}
                <!-- Cooldown ring -->
...
<style>
  ...
  .ve-blob--2 {
    ...
    animation: ve-float 10s ease-in-out infinite reverse;
  }
  ...
  .ve-card {
    ...
    overflow: hidden;
    animation: ve-rise .5s cubic-bezier(.16,1,.3,1) both;
  }

  /* Envelope illustration */
  .ve-envelope-wrap {
    background: linear-gradient(160deg, #1a0b2e 0%, #2d1b4e 100%);
    padding: 2.5rem 2rem 2rem;
    display: flex; align-items: center; justify-content: center;
    position: relative; overflow: hidden;
  }
  .ve-envelope-wrap::after {
    content: '';
    position: absolute; inset: 0;
    background: radial-gradient(ellipse 80% 60% at 50% 80%, rgba(139,92,246,.2), transparent);
  }

  .ve-envelope {
    position: relative; width: 80px; height: 60px; z-index: 1;
    animation: ve-hover 3s ease-in-out infinite;
  }
  ...
  .ve-title {
    ...
    font-size: 1.75rem; color: #1e1b4b; letter-spacing: -.02em;
    margin: 0; line-height: 1.2;
  }

  .ve-subtitle {
    font-size: .9rem; color: #64748b; line-height: 1.6; margin: 0;
  }

  .ve-email {
    color: #1e1b4b; font-weight: 700;
    background: #f3e8ff; padding: .1rem .4rem; border-radius: .375rem;
  }
  ...
  .ve-step-num {
    ...
    display: flex; align-items: center; justify-content: center;
    box-shadow: 0 2px 6px rgba(106,44,145,.25);
  }

  .ve-step-text {
    font-size: .8125rem; color: #374151; font-weight: 500;
  }
  ...
  .ve-resend-row {
    ...
    border: 1.5px solid #e9d5ff; border-radius: .875rem;
  }
  ...
  .ve-resend-btn {
    ...
    padding: .5rem .875rem; border-radius: .625rem; border: none;
    background: linear-gradient(135deg, #6a2c91, #4a1d6e);
  ...
  .ve-resend-btn:hover:not(.ve-resend-btn--disabled) {
  ...
  .ve-cooldown-num {
    font-size: .55rem; font-weight: 800; color: #6a2c91;
    position: relative; z-index: 1;
    line-height: 1;
  }
  .ve-resend-btn--disabled .ve-cooldown-num { color: #94a3b8; }
  .ve-resend-btn--disabled circle:last-child { stroke: #c4b5fd; }
  ...
  .ve-mail-client {
    display: inline-flex; align-items: center; gap: .375rem;
    padding: .4375rem .875rem; border-radius: .625rem;
    border: 1.5px solid #e5e7eb; background: white;
    font-size: .75rem; font-weight: 600; color: #374151;
    text-decoration: none; transition: all .2s;
  }
  .ve-mail-client:hover {
  ...
  .ve-footer {
    ...
    animation: ve-rise .5s .15s cubic-bezier(.16,1,.3,1) both;
  }
  ...
  .ve-home-btn {
    ...
    font-size: .8rem; color: #94a3b8; font-family: 'DM Sans', sans-serif;
  ...
  @media (max-width: 480px) {
    .ve-card { border-radius: 1.25rem; }
    .ve-card-content { padding: 1.5rem 1.25rem 1.75rem; }
    .ve-title { font-size: 1.5rem; }
    .ve-mail-clients { justify-content: center; }
    .ve-resend-row { flex-direction: column; gap: .75rem; align-items: flex-start; }
    .ve-resend-btn { width: 100%; justify-content: center; }
  }
</style>

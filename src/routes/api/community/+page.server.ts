...
export const load: PageServerLoad = async ({ locals, url, setHeaders }) => {
  ...
  // Add cache headers to prevent excessive reloads
  setHeaders({
  ...
  try {
    const [
      ...
      db.select({
        ...
        .where(
          lat && lng
            ? or(eq(communityPosts.scope, 'global'),
      ...
      db.select({
        ...
        .where(
          lat && lng
            ? or(eq(communityDiscussions.scope, 'global'),
      ...
      db.select({
        ...
        .where(and(gt(communityEvents.startsAt, new Date()), eq(communityEvents.isCancelled, false)))
    ...
    return {
      posts: postsData.map(p => ({
        ...
        authorName: p.isAnonymous
          ? 'Anonymous'
          : [safeRevealField(p.authorFirstName), safeRevealField(p.authorLastName)]
      ...
      discussions: discussionsData.map(d => ({
        ...
        authorName: [safeRevealField(d.authorFirstName), safeRevealField(d.authorLastName)]
          .filter(Boolean).join(' ') || 'Unknown',
      })),
      ...
      members: membersData.map(m => ({
        ...
        name:          [safeRevealField(m.firstName), safeRevealField(m.lastName)]
                         .filter(Boolean).join(' ') || 'Unknown',
...
export const actions: Actions = {
  createPost: async ({ request, locals }) => {
    ...
    await db.insert(communityPosts).values({
      userId: locals.user.id, content: content.trim(),
      category: category as any, scope: scope as any,
      isAnonymous, lat, lng, locationName,
    });
    return { success: true };
  },
  ...
  toggleLike: async ({ request, locals }) => {
    ...
    const existing = await db.query.communityPostLikes.findFirst({
      where: and(eq(communityPostLikes.postId, postId), eq(communityPostLikes.userId, locals.user.id)),
    });
    if (existing) {
      await db.delete(communityPostLikes).where(eq(communityPostLikes.id, existing.id));
      await db.update(communityPosts).set({ likeCount: sql`${communityPosts.likeCount} - 1` }).where(eq(communityPosts.id, postId));
      return { liked: false };
    } else {
  ...
  toggleFollow: async ({ request, locals }) => {
    ...
    const existing = await db.query.userFollows.findFirst({
      where: and(eq(userFollows.followerId, locals.user.id), eq(userFollows.followedId, followedId)),
    });
    if (existing) {
  ...
  toggleRsvp: async ({ request, locals }) => {
    ...
    const existing = await db.query.communityEventAttendees.findFirst({
      where: and(eq(communityEventAttendees.eventId, eventId), eq(communityEventAttendees.userId, locals.user.id)),
    });
    if (existing) {
      await db.delete(communityEventAttendees).where(eq(communityEventAttendees.id, existing.id));
      await db.update(communityEvents).set({ attendeeCount: sql`${communityEvents.attendeeCount} - 1` }).where(eq(communityEvents.id, eventId));
      return { attending: false };
    } else {
      const event = await db.query.communityEvents.findFirst({ where: eq(communityEvents.id, eventId) });
      if (event?.maxAttendees && event.attendeeCount >= event.maxAttendees) return fail(400, { error: 'Event is full' });
